...
void uBlock_256_KeySchedule(unsigned char *key, int plen)
{
	...
	__m128i c5 = _mm_setr_epi8(10,5,15,0,2,7,8,13,14,6,4,12,1,3,11,9);	
...
int Crypt_Enc_Block(unsigned char *input, int in_len, unsigned char *output, int *out_len, unsigned char *key, int keylen)
{
	...
	uBlock_256_KeySchedule(key, 16);

	for (g = 0; g < in_len / 16; g++)
	{
		uBlock_128_Encrypt(input+g*16, output+g*16, 24);
	}
...
int Crypt_Dec_Block(unsigned char *input,int in_len, unsigned char *output,int *out_len, unsigned char *key, int keylen)
{
	...
	for (g = 0; g < in_len / 16; g++)
	{
		uBlock_128_Decrypt(input+g*16, output+g*16, 24);
	}
	*out_len = in_len - in_len % 16;
...
int Key_Schedule(unsigned char *seedkey, int keylen, int direction, unsigned char *subkey)
{
   ...
   uBlock_256_KeySchedule(seedkey, 16);
   ...
   if(direction == 1)
   {
	   for(i = 0; i < 25; i++)
	   {
		   for(j = 0; j < 16; j++)
		   {
			   subkey[i * 16 + j] = (Subkey[24 - i][2 * j] << 4) ^ Subkey[24 - i][2 * j + 1];
		   }
	   }
   }
...
int Crypt_Enc_Block_CBC(unsigned char *input, int in_len, unsigned char *output, int *out_len, unsigned char *key, int keylen)
{
	...
	for (g = 0; g < in_len / 16; g++)
	{
		...
		uBlock_128_Encrypt(iv, output+g*16, 24);
...
void main()
{
	...
	unsigned char cipher[16], subkey[400];

	Crypt_Enc_Block(plain, 16, cipher, &j, key, 32);
	for(i = 0; i < 16; i++)
	{
		printf("%02x ", cipher[i]);
	}
	...
	Crypt_Dec_Block(cipher, 16, plain, &j, key, 32);
	for(i = 0; i < 16; i++)
	{
		printf("%02x ", plain[i]);
	}
	...
	for(r = 1; r <= 24; r++)
	{
		Crypt_Enc_Block_Round(plain, 16, cipher, &j, key, 32, r);	
		for(i = 0; i < 16; i++)
		{
			printf("%02x ", cipher[i]);
		}
