...
unsigned char long_plain_1[ARRAY_SIZE_MAX] = {1}, long_plain_2[ARRAY_SIZE_MAX] = {2}, long_plain_3[ARRAY_SIZE_MAX] = {0}, long_plain_4[ARRAY_SIZE_MAX] = {0};
...
int Crypt_Enc_Block(unsigned char *input_1, unsigned char *input_2, int in_len, unsigned char *output_1, unsigned char *output_2, int *out_len, unsigned char *key_1, unsigned char *key_2, int keylen)
{
    int g;
    ...
    {
        uBlock_128_Encrypt(input_1+g*16, input_2+g*16, output_1+g*16, output_2+g*16, 16);
    }
...
int Crypt_Dec_Block(unsigned char *input_1, unsigned char *input_2, int in_len, unsigned char *output_1, unsigned char *output_2, int *out_len, unsigned char *key_1, unsigned char *key_2, int keylen)
{
    int g;
    ...
    for (g = 0; g < in_len / 16; g++)
    {
        uBlock_128_Decrypt(input_1+g*16, input_2+g*16, output_1+g*16, output_2+g*16, 16);
    }
    *out_len = in_len - in_len % 16;
...
int Key_Schedule(unsigned char *seedkey, int keylen, int direction, unsigned char *subkey)
{
    ...
    if(direction == 1)
    {
        for(i = 0; i < 17; i++)
        {
            for(j = 0; j <32; j++)
            {
                subkey[i * 32 + j] = (Subkey[16 - i][2 * j] << 4) ^ Subkey[16 - i][2 * j + 1];
            }
        }
    }
...
int Crypt_Enc_Block_CBC(unsigned char *input_1, unsigned char *input_2, int in_len, unsigned char *output_1, unsigned char *output_2, int *out_len, unsigned char *key_1, unsigned char *key_2, int keylen)
{
    int g, j;
...
int Crypt_Dec_Block_CBC(unsigned char *input_1, unsigned char *input_2, int in_len, unsigned char *output_1, unsigned char *output_2, int *out_len, unsigned char *key_1, unsigned char *key_2, int keylen)
{
    int g, j;
...
{
    int i, j, r;
    unsigned char key_1[16]   = {0x0C,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xFE,0xDC,0xBA,0x98,0x76,0x54,0x32,0x10};
    unsigned char key_2[16]   = {0x0D,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xFE,0xDC,0xBA,0x98,0x76,0x54,0x32,0x10};
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        {
            Crypt_Enc_Block(plain_1, plain_2, 16, cipher_1, cipher_2, &j, key_1, key_2, 16);
        }
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
        printf("%f\n", 2 * (double)total_bits / (times * 1024 * 1024));
        total += 2 * (double)total_bits / (times * 1024 * 1024);
    }
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        for(r = 0; r < 10000000; r++)
        {
            Crypt_Dec_Block(cipher_1, cipher_2, 16, plain_1, plain_2, &j, key_1, key_2, 16);
        }
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
...
/////////////------------(1 + n) long text test----------/////////////////
    ...
    for(i = 0; i < 10; i++)
    {
        op = clock();
        for(r = 0; r < 1024; r ++)
            Crypt_Enc_Block(long_plain_1, long_plain_2, 262144, long_cipher_1, long_cipher_2, &j, key_1, key_2, 16);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
    ...
    for(i = 0; i < 10; i++)
    {
        op = clock();
        for(r = 0; r< 1024; r ++)
            Crypt_Dec_Block(long_cipher_1, long_cipher_2, 262144, long_plain_1, long_plain_2, &j, key_1, key_2, 16);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
        printf("%f mbps\n", (double)2 * total_mb / times);
        total += (double)2 * total_mb / times;
...
/////////------------CBC test----------/////////////////
    ...
    for(i = 0; i < 10; i++)
    {
        op = clock();
        for(r = 0; r < 1024; r ++)
            Crypt_Enc_Block_CBC(long_plain_1, long_plain_2, 262144, long_cipher_1, long_cipher_2, &j, key_1, key_2, 16);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
        printf("%f mbps\n", (double)2 * total_mb / times);
        total += (double)2 * total_mb / times;
    ...
    for(i = 0; i < 10; i++)
    {
        op = clock();
        for(r = 0; r < 1024; r ++)
            Crypt_Dec_Block_CBC(long_cipher_1, long_cipher_2, 262144, long_plain_1, long_plain_2, &j, key_1, key_2, 16);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
        printf("%f mbps\n", (double)2 * total_mb / times);
        total += (double)2 * total_mb / times;
