...
inline void uBlock_256_KeySchedule(unsigned char *key) {
  ...
  __m256i PK =
      _mm256_setr_epi8(10, 5, 15, 0, 2, 7, 8, 13, 1, 14, 4, 12, 9, 11, 3, 6, 8,
                       9, 10, 11, 12, 13, 14, 15, 0, 1, 2, 3, 4, 5, 6, 7);
  __m256i S_first_SK_second = _mm256_setr_epi8(
      0x7, 0x4, 0x9, 0xc, 0xb, 0xa, 0xd, 0x8, 0xf, 0xe, 0x1, 0x6, 0x0, 0x3, 0x2,
      0x5, 0, 2, 4, 6, 8, 10, 12, 14, 3, 1, 7, 5, 11, 9, 15, 13);

  __m256i state1, state2, state3, state4, tmp1, tmp2, k;

  // divide high low
  state2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(key));
  state4 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(key + 32));
...
inline void uBlock_256_Encrypt(unsigned char *plain, unsigned char *cipher,
                               int round) {
  int i = 0;
  ...
  __m256i L1_front = _mm256_setr_epi8(
      0x74, 0x75, 0x7e, 0x7f, 0x80, 0x81, 0x8a, 0x8b, 0x76, 0x77, 0x7c, 0x7d,
      0x82, 0x83, 0x88, 0x89, 0xf2, 0xf3, 0xf8, 0xf9, 0x0e, 0x0f, 0x04, 0x05,
      0x0c, 0x0d, 0x06, 0x07, 0xfa, 0xfb, 0xf0, 0xf1);
  __m256i L1_rear = _mm256_setr_epi8(
      0xf4, 0xf5, 0xfe, 0xff, 0x00, 0x01, 0x0a, 0x0b, 0xf6, 0xf7, 0xfc, 0xfd,
      0x02, 0x03, 0x08, 0x09, 0x72, 0x73, 0x78, 0x79, 0x8e, 0x8f, 0x84, 0x85,
      0x8c, 0x8d, 0x86, 0x87, 0x7a, 0x7b, 0x70, 0x71);
  __m256i L2_front = _mm256_setr_epi8(
      0x7c, 0x7d, 0x86, 0x87, 0x72, 0x73, 0x88, 0x89, 0x82, 0x83, 0x78, 0x79,
      0x74, 0x75, 0x8e, 0x8f, 0xfe, 0xff, 0xf0, 0xf1, 0x0a, 0x0b, 0x04, 0x05,
      0x0c, 0x0d, 0xf6, 0xf7, 0x00, 0x01, 0xfa, 0xfb);
  __m256i L2_rear = _mm256_setr_epi8(
      0xfc, 0xfd, 0x06, 0x07, 0xf2, 0xf3, 0x08, 0x09, 0x02, 0x03, 0xf8, 0xf9,
      0xf4, 0xf5, 0x0e, 0x0f, 0x7e, 0x7f, 0x70, 0x71, 0x8a, 0x8b, 0x84, 0x85,
      0x8c, 0x8d, 0x76, 0x77, 0x80, 0x81, 0x7a, 0x7b);

  __m256i S =
      _mm256_setr_epi8(0x7, 0x4, 0x9, 0xc, 0xb, 0xa, 0xd, 0x8, 0xf, 0xe, 0x1,
                       0x6, 0x0, 0x3, 0x2, 0x5, 0x7, 0x4, 0x9, 0xc, 0xb, 0xa,
                       0xd, 0x8, 0xf, 0xe, 0x1, 0x6, 0x0, 0x3, 0x2, 0x5);
  ...
  hi_2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(plain + 32));
  ...
  _mm256_storeu_si256(reinterpret_cast<__m256i *>(cipher), state1);
  _mm256_storeu_si256(reinterpret_cast<__m256i *>(cipher + 32), hi_1);
}

inline void uBlock_256_Decrypt(unsigned char *cipher, unsigned char *plain,
                               int round) {
  int i = 0;
  ...
  __m256i c5_to_c8 =
      _mm256_setr_epi8(0, 2, 4, 6, 8, 10, 12, 14, 1, 3, 5, 7, 9, 11, 13, 15, 0,
                       2, 4, 6, 8, 10, 12, 14, 1, 3, 5, 7, 9, 11, 13, 15);
  __m256i L1_Inv_front = _mm256_setr_epi8(
      0x8e, 0x8f, 0x80, 0x81, 0x70, 0x71, 0x78, 0x79, 0x82, 0x83, 0x8c, 0x8d,
      0x7a, 0x7b, 0x72, 0x73, 0xf4, 0xf5, 0xfc, 0xfd, 0x06, 0x07, 0x0a, 0x0b,
      0xfe, 0xff, 0xf6, 0xf7, 0x08, 0x09, 0x04, 0x05);
  __m256i L1_Inv_rear = _mm256_setr_epi8(
      0x0e, 0x0f, 0x00, 0x01, 0xf0, 0xf1, 0xf8, 0xf9, 0x02, 0x03, 0x0c, 0x0d,
      0xfa, 0xfb, 0xf2, 0xf3, 0x74, 0x75, 0x7c, 0x7d, 0x86, 0x87, 0x8a, 0x8b,
      0x7e, 0x7f, 0x76, 0x77, 0x88, 0x89, 0x84, 0x85);
  __m256i L2_Inv_front = _mm256_setr_epi8(
      0x82, 0x83, 0x74, 0x75, 0x7c, 0x7d, 0x8a, 0x8b, 0x7a, 0x7b, 0x8e, 0x8f,
      0x70, 0x71, 0x80, 0x81, 0x0c, 0x0d, 0xf8, 0xf9, 0x06, 0x07, 0xf2, 0xf3,
      0xf6, 0xf7, 0x04, 0x05, 0x08, 0x09, 0xfe, 0xff);
  __m256i L2_Inv_rear = _mm256_setr_epi8(
      0x02, 0x03, 0xf4, 0xf5, 0xfc, 0xfd, 0x0a, 0x0b, 0xfa, 0xfb, 0x0e, 0x0f,
      0xf0, 0xf1, 0x00, 0x01, 0x8c, 0x8d, 0x78, 0x79, 0x86, 0x87, 0x72, 0x73,
      0x76, 0x77, 0x84, 0x85, 0x88, 0x89, 0x7e, 0x7f);
  __m256i S_Inv =
      _mm256_setr_epi8(0xc, 0xa, 0xe, 0xd, 0x1, 0xf, 0xb, 0x0, 0x7, 0x2, 0x5,
                       0x4, 0x3, 0x6, 0x9, 0x8, 0xc, 0xa, 0xe, 0xd, 0x1, 0xf,
                       0xb, 0x0, 0x7, 0x2, 0x5, 0x4, 0x3, 0x6, 0x9, 0x8);
  __m256i A1 = _mm256_setr_epi8(1, 2, 3, 4, 5, 6, 7, 0, 9, 10, 11, 12, 13, 14,
                                15, 8, 17, 18, 19, 20, 21, 22, 23, 16, 25, 26,
                                27, 28, 29, 30, 31, 24);
  __m256i A2 = _mm256_setr_epi8(2, 3, 4, 5, 6, 7, 0, 1, 10, 11, 12, 13, 14, 15,
  ...
  hi_2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(cipher + 32));
  hi_1 = _mm256_srli_epi16(hi_2, 4);
  hi_1 = _mm256_and_si256(hi_1, con);
  hi_2 = _mm256_and_si256(hi_2, con);
  ...
  _mm256_storeu_si256(reinterpret_cast<__m256i *>(plain), state1);
  _mm256_storeu_si256(reinterpret_cast<__m256i *>(plain + 32), hi_1);
}

inline int Crypt_Enc_Block(unsigned char *input, int in_len,
...
inline int Crypt_Dec_Block(unsigned char *input, int in_len,
                           unsigned char *output, unsigned char *key) {
  uBlock_256_KeySchedule(key);
...
inline int Crypt_Enc_Block_CBC(unsigned char *input, int in_len,
                               unsigned char *output, unsigned char *key) {
  int g, j;
  ...
  for (g = 0; g < in_len / 64; g++) {
    for (j = 0; j < 64; j++) iv[j] ^= input[g * 64 + j];
...
inline int Crypt_Dec_Block_CBC(unsigned char *input, int in_len,
                               unsigned char *output, unsigned char *key) {
  int g, j;
  unsigned char iv[64] = {0};
...
int main() {
  ...
  unsigned char key[64] = {
      ...
      0x76, 0x54, 0x32, 0x10, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
      0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
  unsigned char plain_1[64] = {
      0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0xFE, 0xDC, 0xBA,
      0x98, 0x76, 0x54, 0x32, 0x10, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
      0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x00,
      0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
      0x0c, 0x0d, 0x0e, 0x0f, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09,
      0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00};
  unsigned char cipher_1[64];

  printf("\n10000000-round-Encryption:\n");
  u_int32_t op, ed, total_bits = 2560000000;
  double times, total = 0;
  for (i = 0; i < 10; i++) {
    op = clock();
    for (r = 0; r < 10000000; r++) Crypt_Enc_Block(plain_1, 64, cipher_1, key);
    ed = clock();
    times = static_cast<double>(ed - op) / CLOCKS_PER_SEC;
    printf("%f\n", 2 * static_cast<double>(total_bits / (times * 1024 * 1024)));
    total += static_cast<double>(2 * total_bits / (times * 1024 * 1024));
  }

  printf("cipher1:\n");
  for (i = 0; i < 32; i++) printf("%02X ", cipher_1[i]);
  printf("\n");
  printf("cipher2:\n");
  for (i = 0; i < 32; i++) printf("%02X ", cipher_1[i + 32]);
  printf("\n");
  ...
  for (i = 0; i < 10; i++) {
    op = clock();
    for (r = 0; r < 10000000; r++) Crypt_Dec_Block(cipher_1, 64, plain_1, key);
    ed = clock();
    times = static_cast<double>(ed - op) / CLOCKS_PER_SEC;
    printf("%f\n", 2 * static_cast<double>(total_bits / (times * 1024 * 1024)));
    total += static_cast<double>(2 * total_bits / (times * 1024 * 1024));
  ...
  printf("plain1:\n");
  for (i = 0; i < 32; i++) printf("%02X ", plain_1[i]);
  printf("\n");
  printf("plain2:\n");
  for (i = 0; i < 32; i++) printf("%02X ", plain_1[i + 32]);
  printf("\n");
