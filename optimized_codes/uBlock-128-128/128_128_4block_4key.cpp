...
unsigned char Subkey_1[17][32], Subkey_2[17][32], Subkey_3[17][32],
    Subkey_4[17][32];
...
inline void uBlock_128_KeySchedule(unsigned char *key_1, unsigned char *key_2,
                                   int ctr) {
  int i;
  ...
  state2 = _mm256_loadu2_m128i(reinterpret_cast<__m128i *>(key_1),
                               reinterpret_cast<__m128i *>(key_2));
  ...
  if (ctr == 0) {
    _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_1[0]), t1);
    _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_2[0]), t2);
  } else {
    _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_3[0]), t1);
    _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_4[0]), t2);
  }
  ...
  for (i = 1; i <= 16; i++) {
    ...
    if (ctr == 0) {
      _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_1[i]), t1);
      _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_2[i]), t2);
    } else {
      _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_3[i]), t1);
      _mm256_storeu_si256(reinterpret_cast<__m256i *>(Subkey_4[i]), t2);
    }
  }
}
...
inline void uBlock_128_Encrypt(unsigned char *plain_1, unsigned char *plain_2,
  ...
  for (i = 0; i < round; i++) {
    ...
    k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_3[i]));
    ...
    k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_4[i]));
    k2 = _mm256_slli_epi16(k2, 4);
    k = _mm256_xor_si256(k1, k2);
  ...
  k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_3[round]));
  k2 = _mm256_slli_epi16(k2, 4);
  k = _mm256_xor_si256(k1, k2);
  t1 = _mm256_xor_si256(t1, k);

  k1 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_2[round]));
  k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_4[round]));
  k2 = _mm256_slli_epi16(k2, 4);
  k = _mm256_xor_si256(k1, k2);
  t2 = _mm256_xor_si256(t2, k);
...
inline void uBlock_128_Decrypt(unsigned char *cipher_1, unsigned char *cipher_2,
  ...
  k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_3[0]));
  k2 = _mm256_slli_epi16(k2, 4);
  k = _mm256_xor_si256(k1, k2);
  t1 = _mm256_xor_si256(t1, k);

  k1 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_2[0]));
  k2 = _mm256_loadu_si256(reinterpret_cast<__m256i *>(Subkey_4[0]));
  k2 = _mm256_slli_epi16(k2, 4);
  k = _mm256_xor_si256(k1, k2);
  t2 = _mm256_xor_si256(t2, k);
...
inline int Crypt_Enc_Block(unsigned char *input_1, unsigned char *input_2,
  ...
  for (int g = 0; g < in_len / 16; g++)
    uBlock_128_Encrypt(input_1 + g * 16, input_2 + g * 16, output_2 + g * 16,
                       output_1 + g * 16, input_3 + g * 16, input_4 + g * 16,
                       output_4 + g * 16, output_3 + g * 16, 16);
...
inline int Crypt_Dec_Block(unsigned char *input_1, unsigned char *input_2,
  ...
  for (int g = 0; g < in_len / 16; g++)
    uBlock_128_Decrypt(input_1 + g * 16, input_2 + g * 16, output_2 + g * 16,
                       output_1 + g * 16, input_3 + g * 16, input_4 + g * 16,
                       output_4 + g * 16, output_3 + g * 16, 16);
...
inline int Crypt_Enc_Block_CBC(unsigned char *input_1, unsigned char *input_2,
  ...
  for (g = 0; g < in_len / 16; g++) {
    ...
    uBlock_128_Encrypt(iv_1, iv_2, output_2 + g * 16, output_1 + g * 16, iv_3,
                       iv_4, output_4 + g * 16, output_3 + g * 16, 16);
...
int main() {
  unsigned char key_1[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                             0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
  unsigned char key_2[16] = {0x02, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                             0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
  unsigned char key_3[16] = {0x03, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                             0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
  unsigned char key_4[16] = {0x04, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                             0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10};
