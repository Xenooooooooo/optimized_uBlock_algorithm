...
inline void uBlock_256_KeySchedule(unsigned char *key)
{
    ...
    _mm256_storeu_si256((__m256i*)Subkey_1[0], t3);
    _mm256_storeu_si256((__m256i*)Subkey_2[0], t4);
    ...
    for (i = 1; i <= 24; i++)
    {
        ...
        t4 = _mm256_slli_epi16(state2, 4);
        t4 = _mm256_xor_si256(state2, t4);
        _mm256_storeu_si256((__m256i*)Subkey_1[i], t3);
        _mm256_storeu_si256((__m256i*)Subkey_2[i], t4);
    }
}
...
inline void uBlock_128_Encrypt(unsigned char *plain_1, unsigned char *plain_2, unsigned char *cipher_1, unsigned char *cipher_2, unsigned char *plain_3, unsigned char *plain_4, unsigned char *cipher_3, unsigned char *cipher_4, int round)
{
    ...
    __m256i state1, state2, state3, state4, state5, state6, t1, t2, t3, t4, k, k1, k2, k3, con = _mm256_set1_epi8(0x0f), con1 = _mm256_set1_epi8(0xf0);
    ...
    _mm256_storeu2_m128i((__m128i*)cipher_1, (__m128i*)cipher_2, state5);
    ...
    state5 = _mm256_xor_si256(state5, state6);
    state5 = _mm256_permute4x64_epi64(state5, 0b11011000);
    _mm256_storeu2_m128i((__m128i*)cipher_3, (__m128i*)cipher_4, state5);
}
...
inline void uBlock_128_Decrypt(unsigned char *cipher_1, unsigned char *cipher_2, unsigned char *plain_1, unsigned char *plain_2, unsigned char *cipher_3, unsigned char *cipher_4, unsigned char *plain_3, unsigned char *plain_4, int round)
{
    ...
    for (i = round; i > 0; i--)
    {
        k = _mm256_loadu_si256((__m256i*)Subkey_1[i]);
        t1 = _mm256_xor_si256(t1, k);
        k = _mm256_loadu_si256((__m256i*)Subkey_2[i]);
        t2 = _mm256_xor_si256(t2, k);
    ...
    state5 = _mm256_xor_si256(state5, state6);
    state5 = _mm256_permute4x64_epi64(state5, 0b11011000);
    _mm256_storeu2_m128i((__m128i*)plain_1, (__m128i*)plain_2, state5);
    ...
    state5 = _mm256_xor_si256(state5, state6);
    state5 = _mm256_permute4x64_epi64(state5, 0b11011000);
    _mm256_storeu2_m128i((__m128i*)plain_3, (__m128i*)plain_4, state5);
}

inline int Crypt_Enc_Block(unsigned char *input_1, unsigned char *input_2, unsigned char *input_3, unsigned char *input_4, int in_len, unsigned char *output_1, unsigned char *output_2, unsigned char *output_3, unsigned char *output_4, unsigned char *key)
{
...
inline int Crypt_Dec_Block(unsigned char *input_1, unsigned char *input_2, unsigned char *input_3, unsigned char *input_4, int in_len, unsigned char *output_1, unsigned char *output_2, unsigned char *output_3, unsigned char *output_4, unsigned char *key)
{
    uBlock_256_KeySchedule(key);
...
int main()
{
    ...
    unsigned char plain_2[16]   = {0x02,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xFE,0xDC,0xBA,0x98,0x76,0x54,0x32,0x10};
    unsigned char plain_3[16]   = {0x03,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xFE,0xDC,0xBA,0x98,0x76,0x54,0x32,0x10};
    unsigned char plain_4[16]   = {0x04,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF,0xFE,0xDC,0xBA,0x98,0x76,0x54,0x32,0x10};
...
///////////////-------(1 + 1) * n speed testing-------///////////////////
    ...
    for(i = 0; i < 10; i++)
    {
        ...
        for (r = 0; r < 10000000; r++)
            Crypt_Enc_Block(plain_1, plain_2, plain_3, plain_4, 16, cipher_1, cipher_2, cipher_3, cipher_4, key);
        ed = clock();
...
/////////////-----------------------------/////////////////
    ...
    for(i = 0; i < 10; i++)
    {
        op = clock();
        for (r = 0; r < 10000000; r++)
            Crypt_Dec_Block(cipher_1, cipher_2, cipher_3, cipher_4, 16, plain_1, plain_2, plain_3, plain_4, key);
        ed = clock();
        times = (double) (ed - op) / CLOCKS_PER_SEC;
        printf("%f\n", 4 * (double) total_bits / (times * 1024 * 1024));
        total += 4 * (double) total_bits / (times * 1024 * 1024);
...
/////////////------------(1 + n) long text test----------/////////////////
    ...
    for(i = 0; i < 10; i ++)
    {
        op = clock();
        for(r = 0; r < 1024; r ++)
            Crypt_Enc_Block(long_plain_1, long_plain_2, long_plain_3, long_plain_4, 262144, long_cipher_1, long_cipher_2, long_cipher_3, long_cipher_4, key);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
...
/////////////-----------------------------/////////////////
    ...
    for(i = 0; i < 10; i ++)
    {
        op = clock();
        for(r = 0; r < 1024; r ++)
            Crypt_Dec_Block(long_cipher_1, long_cipher_2, long_cipher_3, long_cipher_4, 262144, long_plain_1, long_plain_2, long_plain_3, long_plain_4, key);
        ed = clock();
        times = (double)(ed - op) / CLOCKS_PER_SEC;
        printf("%f\n", (double)total_mb / times);
        total += (double)total_mb / times;
